...
function JackknifeVector(jk::JackknifeVector, f::Function)
    Base.depwarn("JackknifeVector(jk::JackknifeVector, f::Function) is deprecated. Use JackknifeVector(f, jk) instead.",
                 :JackknifeVector)
...
for op in (:+, :-)
    @eval Base.$op(jk::JackknifeVector, rhs::Real) = JackknifeVector(lhs -> ($op)(lhs, rhs),
                                                                     jk)
    @eval Base.$op(jk::JackknifeVector, rhs::Vector) = JackknifeVector(lhs -> ($op)(lhs,
                                                                                    rhs),
                                                                       jk)
    @eval Base.$op(lhs::Real, jk::JackknifeVector) = JackknifeVector(rhs -> ($op)(lhs, rhs),
                                                                     jk)
    @eval Base.$op(lhs::Vector, jk::JackknifeVector) = JackknifeVector(rhs -> ($op)(lhs,
                                                                                    rhs),
                                                                       jk)
    ...
    @eval Base.$op(lhs::JackknifeVector, rhs::JackknifeVector) = JackknifeVector($op, lhs,
                                                                                 rhs)
    ...
    @eval broadcast(::typeof($op), jk::JackknifeVector, rhs::Vector) = JackknifeVector(lhs -> ($op)(lhs,
                                                                                                    rhs),
                                                                                       jk)
    @eval broadcast(::typeof($op), lhs::Real, jk::JackknifeVector) = JackknifeVector(rhs -> ($op)(lhs,
                                                                                                  rhs),
                                                                                     jk)
    @eval broadcast(::typeof($op), lhs::Vector, jk::JackknifeVector) = JackknifeVector(rhs -> ($op)(lhs,
                                                                                                    rhs),
                                                                                       jk)
    @eval broadcast(::typeof($op), lhs::Jackknife, rhs::JackknifeVector) = JackknifeVector($op,
                                                                                           lhs,
                                                                                           rhs)
    @eval broadcast(::typeof($op), lhs::JackknifeVector, rhs::Jackknife) = JackknifeVector($op,
                                                                                           lhs,
                                                                                           rhs)
    @eval broadcast(::typeof($op), lhs::JackknifeVector, rhs::JackknifeVector) = JackknifeVector($op,
                                                                                                 lhs,
                                                                                                 rhs)
end
for op in (:*, :/, :\)
