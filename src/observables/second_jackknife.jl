...
for op in binary_functions
    eval(Expr(:import, :Base, op))
    eval(Expr(:export, op))
    @eval ($op)(jk::SecondJackknife, rhs::Real) = SecondJackknife(jk,
                                                                  lhs -> ($op)(lhs, rhs))
    @eval ($op)(lhs::Real, jk::SecondJackknife) = SecondJackknife(jk,
                                                                  rhs -> ($op)(lhs, rhs))
    op_bw = symbol("." * string(op))
    @eval ($op)(lhs::SecondJackknife, rhs::SecondJackknife) = SecondJackknife(($op_bw)(lhs.xs,
                                                                                       rhs.xs))
end
