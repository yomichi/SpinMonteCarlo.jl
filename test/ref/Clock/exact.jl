...
for N in Ns
    for Q in Qs
        ...
        for c in ("x", "y")
            res["Susceptibility $c"] = N .* (res["Magnetization $c^2"] .-
                                             res["Magnetization $c"] .^ 2) .* betas
            res["Connected Susceptibility $c"] = N .* (res["Magnetization $c^2"] .-
                                                       res["|Magnetization $c|"] .^ 2) .*
                                                 betas
            res["Binder Ratio $c"] = res["Magnetization $c^4"] ./
                                     (res["Magnetization $c^2"] .^ 2)
        end
        res["Susceptibility"] = N .* res["|Magnetization|^2"] .* betas
        res["Connected Susceptibility"] = N .* (res["|Magnetization|^2"] .-
                                                res["|Magnetization|"] .^ 2) .* betas
